...
const crearCollection = async (req, res) => {
  try {
    ...
    const new_collection = await CollectionSchema(nueva_colleccion).save();
...
const actualizarCollection= async (req, res) => {
  try {
      ...
      const collection_actualizada = await CollectionSchema.findByIdAndUpdate (id, informacion_nueva, { new: true })
      return res.json ({
      ok: true,
